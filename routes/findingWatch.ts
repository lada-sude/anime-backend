...
// import { verifyToken } from "../utils/authMiddleware"; // optional
// import { requireAdmin } from "./adminHelpers"; // optional
...
// Public: create (anyone or guests can add)
findingRouter.post("/", async (req, res) => {
try {
const { title, image, episode, note } = req.body;
if (!title || !image) return res.status(400).json({ error: "Missing fields" });


const f = new FindingWatchModel({
title,
image,
...
// Admin: update - left as admin only
findingRouter.put("/:id", /* verifyToken, requireAdmin, */ async (req, res) => {
try {
const { id } = req.params;
const updated = await FindingWatchModel.findOneAndUpdate(
{ $or: [{ id }, { _id: id }] },
req.body,
{ new: true }
);
